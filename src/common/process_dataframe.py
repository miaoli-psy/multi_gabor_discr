...
def insert_new_col_from_three_cols(input_df: pd.DataFrame, old_col1: str, old_col2: str,
                                   old_col3: str, new_col: str,
                                   func_name):
    cols = input_df.columns
    if (old_col1 in cols) and (old_col2 in cols) and (old_col3 in cols):
        input_df[new_col] = input_df.apply(
            lambda x: func_name(x[old_col1], x[old_col2], x[old_col3]), axis=1)
    else:
        raise Exception(f"Warning: missing {old_col1} or {old_col2}")


def insert_new_col_from_four_cols(input_df: pd.DataFrame, old_col1: str, old_col2: str,
                                  old_col3: str, old_col4: str, new_col: str,
                                  func_name):
    cols = input_df.columns
    if (old_col1 in cols) and (old_col2 in cols) and (old_col3 in cols) and (old_col4 in cols):
        input_df[new_col] = input_df.apply(
            lambda x: func_name(x[old_col1], x[old_col2], x[old_col3], x[old_col4]), axis=1)
    else:
        raise Exception(f"Warning: missing {old_col1} or {old_col2} or {old_col3} or {old_col4}")
